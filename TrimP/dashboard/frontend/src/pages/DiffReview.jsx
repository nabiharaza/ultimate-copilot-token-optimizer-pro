...
export default function DiffReview() {
  ...
  useEffect(() => { fetch('/api/copilot/conversations?range=all&limit=50').then(r => r.json()).then(data => { setRows(data); setSelected(current => data.find(row => String(row.id) === String(current?.id)) || data[0] || null) }) }, [refreshTick])
  useEffect(() => { setApproved(false) }, [selected?.id])
